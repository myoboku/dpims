...
public class DBManager
{
    ...
    /**
     * Finds the remedy of a specific problem and returns the remedies to the user
     * @param tableName The name of the table which is to be searched for the remedy
     * @param colName The column name of the problem which contains the ID to be searched
     * @param errorMessage Message to be sent to the in case of an error
    ...
    private static String lookup(String tableName, String colName, String errorMessage, int id, double severity)
    {
        ...
        try
        {
            ...
            // Find all the remedies corresponding to the table, problem Id and severity
            ResultSet resultSet = statement.executeQuery("SELECT Chemical, Dose, Biological, Agronomical FROM " + tableName + " WHERE " + colName + " = " + String.valueOf(id) + " AND " + String.valueOf(severity) + " >= SeverityLowerBound AND " + String.valueOf(severity) + " <= SeverityUpperBound");
            
            // The length of the problem name. Used to eliminate "Remedy" from the table name.
            // When "Remedy" gets eliminated, all we are left with is the problem name.
            // That is, 'PestRemedy' becomes 'Pest' and 'DiseaseRemedy' becomes 'Disease' and is used for further querying.
            ...
            if (resultSet.next())
            {
            	// Get chemical dose from the database
                remedy = (String) resultSet.getObject(1);
                remedy = problem + ": " + remedy + " " + resultSet.getObject(2) + " lit/ha";
                
                // Get biological remedy from the database
                Object bio = resultSet.getObject(3);
                if(bio != null)
                	remedy += ", " + (String) bio;
                
                // Get agronomical solution from the database
                Object agro = resultSet.getObject(4);
                if(agro != null)
                	remedy += ", " + (String) agro;
